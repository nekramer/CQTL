...
CPMadjTMM_invNorm_batchCorrected <- 
  removeBatchEffect(CPMadjTMM_invNorm, batch = covariates %>% pull(RNAextractionKitBatch),
                    batch2 = covariates %>% pull(DNAReagentBatch))
...
write_csv(var_explained, 
          file = paste0("output/pca/", condition, "_PC_RNAextractionKit_DNAReagent_corrected_varExplained.csv"))
...
# Write to file
write_csv(final_pca, 
          file = paste0("output/pca/", condition, "_PCs_RNAextractionKit_DNAReagent_corrected.csv"))