...
# Get min number of significant eGenes ------------------------------------
...
for (tissue in gtex_tissues){
  ...
  # Read in eGenes, filter for significant, and get number
...
# Grab minimum
minimum_eGenes <- num_eGenes %>% filter(num_eGenes == min(num_eGenes)) %>%
  pull(num_eGenes)




# Rank downsample GTEx and compare ----------------------------------------
tissue_percentOverlaps <- list()
for (tissue in gtex_tissues){
  
  # Read in significant eGenes, rank by qvalue, and downsample to minimimum number
  downsampled_tissue <- read_delim(paste0("/pine/scr/n/e/nekramer/GTEx/eqtls/",
                    tissue, ".v8.EUR.egenes.txt.gz"),
             delim = "\t") %>% 
    filter(qval <= 0.05) %>%
    slice_min(order_by = qval, n = minimum_eGenes) %>%
    pull(phenotype_id)
  
  # Remove decimals from ENS IDs
  downsampled_tissue_eGenes <- gsub("\\..*","", downsampled_tissue)
  ...
  CTL_percentOverlap <- list()
    ...
    for (option in CTL_options$Name){
      ...
      percOverlap_FDR <- percentOverlap(eGenes_FDR, downsampled_tissue_eGenes)
      percOverlap_qval <- percentOverlap(eGenes_qval, downsampled_tissue_eGenes)
  ...
  #     percOverlap <- percentOverlap(slice, downsampled_tissue_eGenes)
  #     
...
percentOverlaps_PEERk10_genoPC_batch <- percentOverlaps_all %>%
  filter(Name == "PEER_k10_genoPC_batch") %>%
  group_by(Condition) %>%
  group_split()

CTL <- percentOverlaps_PEERk10_genoPC_batch[[1]] %>% arrange(FDR)
CTL$Tissue <- factor(CTL$Tissue, levels = CTL$Tissue)
FNF <- percentOverlaps_PEERk10_genoPC_batch[[2]]
FNF$Tissue <- factor(FNF$Tissue, levels = CTL$Tissue)

percentOverlaps_PEERk10_genoPC_batch <- bind_rows(CTL, FNF)

ggplot(data = percentOverlaps_PEERk10_genoPC_batch, aes(y = Tissue, x = FDR)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  facet_wrap(~Condition,
             scales = "free_y") +
  theme(axis.title.y = element_blank()) +
  xlab("Total Percent Overlap")

ggsave(filename = "output/plots/GTEx_downsampled_totalpercentOverlapped.pdf")

#saveRDS(tissue_sliced_percentOverlaps, "downsampled_tissue_sliced_percentOverlaps.rds")

tissue_sliced_percentOverlaps <- read_rds("downsampled_tissue_sliced_percentOverlaps.rds")
...
for (tissue in gtex_tissues){
  ...
  t<- ggplot(data = percentOverlaps_data, aes(x = NumGenes, y = FDR, group = Name, color = Name)) +
    geom_line() +
    facet_wrap(~Condition,
               scales = "free_y") +
    xlab("Top Genes") +
  ...
  ggsave(filename = paste0("output/plots/GTEx_", tissue, "_subsampled_sliced_percentOverlap.pdf"))
