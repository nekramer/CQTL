...
def getRsids(variantPositions):
    
    def perVariantRSID(variant):

        regionString = str(variant[0]) + ':' + str(variant[1]) + '-' + str(variant[1])
        rsid = allel.read_vcf('/proj/phanstiel_lab/References/genomes/GENCODE.GRCh38.p13/dbSNP/dbSNP155.GRCh38.p13.vcf.gz', 
                        region = regionString, fields = ['ID'])
